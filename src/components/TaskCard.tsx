...
interface TaskCardProps {
  task: Task;
  onEdit: (task: Task) => void; // Handler for editing the task
  onDelete: (taskId: string) => void; // Handler for deleting the task
}
...
const TaskCard: React.FC<TaskCardProps> = ({ task, onEdit, onDelete }) => {
  ...
  return (
    <div className="bg-white p-5 mt-5 w-80 rounded-2xl shadow-sm flex flex-col gap-5 relative z-10">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="relative">
          ...
          {isMenuOpen && (
            <div className="absolute right-0 mt-2 w-32 bg-white shadow-md rounded-md overflow-hidden z-10">
              <button
                onClick={() => onEdit(task)}
